...
from ..models import Action, Case, Deadline, Decision, Fact, Outcome
from ..schemas import CaseCreate, ChargesInput, FactUpsert, OutcomeInput, ResponseInput, SubmissionInput
from ..services import (analyze_company_response, create_case, diagnose, get_next_question, save_upload, upsert_fact, audit, prepare_claim_package)
...
def serialize_case(db: Session, c: Case):
    ...
    return {
        "id":c.id,"status":c.status,"vertical":c.vertical,"family":c.family,"title":c.title,"raw_intake":c.raw_intake,
        "current_decision_id":c.current_decision_id,"current_action_id":c.current_action_id,"opened_at":c.opened_at,
        "facts":[{"id":f.id,"key":f.key,"value":f.value_json.get("value"),"state":f.state,"user_confirmed":f.user_confirmed} for f in facts],
...
def fact(case_id: str, payload: FactUpsert, db: Session = Depends(get_db)):
    ...
    value=payload.value
    if payload.key=="electricity.supply_end_date" and isinstance(value,str):
        try:
...
def charges(case_id: str, payload: ChargesInput, db: Session = Depends(get_db)):
    ...
    f=upsert_fact(db,c,"electricity.addon.charges",values,state="confirmed",user_confirmed=True)
    return {"fact_id":f.id,"next_question":get_next_question(db,c)}


@router.post("/{case_id}/documents")
async def documents(case_id: str, file: UploadFile = File(...), db: Session = Depends(get_db)):
    c=case_or_404(db,case_id); data=await file.read()
...
def submission(case_id: str, payload: SubmissionInput, db: Session = Depends(get_db)):
    ...
    if settings.legal_holidays_csv:
        for raw in settings.legal_holidays_csv.split(","):
            if raw.strip(): holiday_set.add(date.fromisoformat(raw.strip()))
    ...
    db.add(d); c.status="WAITING_RESPONSE"
...
def response(case_id: str, payload: ResponseInput, db: Session = Depends(get_db)):
    ...
    updated=None
    if c.family=="E04-B" and result["type"] != "UNKNOWN":
        try:
            d,_,_=diagnose(db,c)
            updated=d.to_dict()
        except ValueError:
        ...
        a=Action(case_id=c.id,type="HUMAN_REVIEW",status="OPEN",payload_json={"reason":"UNRECOGNIZED_COMPANY_RESPONSE"})
        db.add(a); c.current_action_id=a.id; c.status="HUMAN_REVIEW"; db.commit()
...
def outcome(case_id: str, payload: OutcomeInput, db: Session = Depends(get_db)):
    ...
    if existing:
        ...
        o=Outcome(case_id=c.id,result_type=payload.result_type); db.add(o)
    o.result_type=payload.result_type; o.amount_recovered=payload.amount_recovered; o.verified_by_user=payload.verified_by_user
    if payload.verified_by_user:
        c.status="RESOLVED"; o.resolved_at=__import__('datetime').datetime.now(__import__('datetime').timezone.utc)
    else:
