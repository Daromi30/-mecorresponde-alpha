...
def evaluate_e05(facts: dict[str, FactValue]) -> EngineResult:
    """E05 — early termination penalty for electricity supply.

    Automation is intentionally limited to the clear no-penalty cases of
    Article 28.3 RD 88/2026. The fixed-price / first-annual-renewal exception
    is routed to human review because direct economic loss and the transitional
    estimation method must be evidenced rather than invented.
    """
    sources = [{
        "title": "Real Decreto 88/2026, art. 28.3 y DT 8ª",
        "url": RD88_URL,
    }]
    missing: list[str] = []
    ...
    exception_applies = bool(fixed_price is True and before_first_renewal is True and not vulnerable_pvpc)
    ...
    open_critical = any(
        c.get("status") == "open" and c.get("impact") == "critical"
        for c in counterarguments
    )
    viability = "MEDIUM" if open_critical else "HIGH"
    reason = (
        "El artículo 28.3 permite a la persona física 2.0TD rescindir el contrato y sus prórrogas sin penalización. "
        "La excepción se limita al contrato a precio fijo antes de la primera prórroga anual. Con los hechos confirmados, esa excepción no concurre."
    )
    if vulnerable_pvpc:
        reason = (
            "El cambio a PVPC acreditando los requisitos de consumidor vulnerable debe realizarse sin penalización ni coste adicional en los términos del artículo 28.9."
        )
    return EngineResult(
        viability=viability, scope_status="SUPPORTED", claimable_amount=penalty,
        economic_value=penalty, worth_pursuing="YES_IF_LOW_COST" if penalty < 30 else "YES",
        reasoning_summary=reason, counterarguments=counterarguments, missing_facts=[],
        next_action="PREPARE_E05_PENALTY_REFUND", rule_result="APPLIES",
        failed_conditions=[],
        calculation={"type": "termination_penalty_refund", "charged_penalty": penalty},
        sources=sources, remedies=["REFUND_TERMINATION_PENALTY", "CEASE_PENALTY_COLLECTION"],
        burden_of_proof=[],
    )
