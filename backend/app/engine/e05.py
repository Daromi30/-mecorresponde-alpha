...
def evaluate_e05(facts: dict[str, FactValue]) -> EngineResult:
    ...
    Automation is intentionally limited to clear no-penalty cases of current
    RD 88/2026. The fixed-price / first-annual-renewal exception goes to human
    review because direct economic loss and the transitional estimation method
    must be evidenced rather than invented.
    """
    sources = [{
    ...
    if natural_person is False or segment_2_0td is False:
        return EngineResult(
            ...
            counterarguments=[],
            missing_facts=[],
            next_action="HUMAN_REVIEW_CONTRACTUAL_PENALTY",
            rule_result="NOT_APPLICABLE",
            failed_conditions=["natural_person_2_0td_scope_failed"],
            calculation=None,
            sources=sources,
            remedies=[],
            burden_of_proof=[],
    ...
    if termination_date and termination_date < CURRENT_RULE_EFFECTIVE:
        return EngineResult(
            ...
            failed_conditions=["termination_before_2026-06-12"],
            calculation=None,
            sources=sources,
            remedies=[],
            burden_of_proof=[],
    ...
    if penalty <= 0:
        return EngineResult(
            ...
            counterarguments=[],
            missing_facts=[],
            next_action="NO_MONETARY_PENALTY_TO_CHALLENGE",
            rule_result="NO_CLAIM",
            failed_conditions=[],
            calculation=None,
            sources=sources,
            remedies=[],
    ...
    # Article 28.9 is its own no-penalty path. Do not ask about a fixed-price
    # exception when the confirmed switch is to PVPC under vulnerable status.
    ...
    if fixed_price is None:
        return EngineResult(
            ...
            counterarguments=[],
            missing_facts=["electricity.fixed_price_contract"],
            next_action="REQUEST_MATERIAL_FACT",
            rule_result="PENDING",
            failed_conditions=[],
            calculation=None,
    ...
    if fixed_price is True and before_first_renewal is None:
        return EngineResult(
            ...
            counterarguments=[],
            missing_facts=["electricity.before_first_annual_renewal"],
            next_action="REQUEST_MATERIAL_FACT",
            rule_result="PENDING",
            failed_conditions=[],
            calculation=None,
    ...
    exception_applies = bool(fixed_price is True and before_first_renewal is True)
    if exception_applies:
        ...
        return EngineResult(
            ...
            counterarguments=[{
                "type": "FIXED_PRICE_FIRST_YEAR_EXCEPTION",
                "status": "confirmed",
                "impact": "critical",
                "origin": "user_fact",
            }],
