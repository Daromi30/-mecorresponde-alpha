...
def evaluate_e03(facts: dict[str, FactValue]) -> EngineResult:
    """E03 — supplier switch without express consent or caused by wrong CUPS."""
    ...
    if switch_date is None:
        ...
        return EngineResult(
            ...
            reasoning_summary="El cambio es anterior al régimen automatizado del RD 88/2026. Debe aplicarse la normativa vigente en la fecha del cambio.",
    ...
    if identity_theft is None:
        ...
        return EngineResult(
            ...
            claimable_amount=None, economic_value=None, worth_pursuing="PROFESSIONAL_REVIEW",
            reasoning_summary="El expediente apunta a posible suplantación de identidad. La restitución del suministro puede seguir siendo procedente, pero la dimensión probatoria y de fraude debe pasar a revisión humana.",
    ...
    if not wrong_cups and consent is True and proof_status == "valid_durable_proof":
        return EngineResult(
            ...
            reasoning_summary="Consta consentimiento expreso respaldado por una prueba duradera válida y el CUPS es correcto. Con esos hechos, E03 no sustenta un cambio no consentido o erróneo.",
    ...
    if not wrong_cups and not no_consent:
        return EngineResult(
            viability="INSUFFICIENT_INFORMATION", scope_status="SUPPORTED",
            claimable_amount=None, economic_value=amount or None, worth_pursuing="NEEDS_INFORMATION",
            reasoning_summary="El usuario reconoce consentimiento pero la prueba duradera no está validada. Antes de concluir debe revisarse la evidencia concreta del consentimiento.",
    ...
    reasoning = (
        f"El cambio presenta {reason}. El artículo 18 exige consentimiento expreso y el artículo 51.3 prevé restituir el punto de suministro al comercializador saliente y al contrato previo cuando el cambio se produjo sin consentimiento o por CUPS incorrecto."
    )
    if amount > 0:
        reasoning += " El artículo 18.7 impide reclamar pago por suministro no solicitado; el importe pagado queda identificado para solicitar su devolución."
    ...
    return EngineResult(
        ...
        calculation=(
            {"type": "unsolicited_supply_paid_amount", "verified_amount_paid": amount,
